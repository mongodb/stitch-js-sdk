import { CoreStitchAppClient, StitchAppClientConfiguration, StitchAppClientInfo, StitchAppRoutes, StitchRequestClient } from "stitch-core";
...
export default class StitchAppClientImpl implements StitchAppClient {
    ...
    public constructor(config: StitchAppClientConfiguration) {  
      ...
      const requestClient =
          new StitchRequestClient(config.baseURL, config.transport);
      this.auth =
          new StitchAuthImpl(
              requestClient, this.routes.authRoutes, config.storage, this.info);
      this.coreClient = new CoreStitchAppClient(this.auth, this.routes);
    }

    public getServiceClientWithName<T>(provider: NamedServiceClientProvider<T>, serviceName: string): T {
      return provider.getClient(
        new StitchServiceImpl(this.auth, this.routes.serviceRoutes, serviceName), this.info);
    }
  
    public getServiceClient<T>(provider: ServiceClientProvider<T>): T {
      return provider.getClient(
        new StitchServiceImpl(this.auth, this.routes.serviceRoutes, ""), this.info);
    }
