...
import { Stitch, StitchAppClient, StitchAppClientConfiguration, UserPasswordAuthProviderClient, UserPasswordCredential } from "stitch-web";
...
export default class BaseStitchIntWebTestHarness extends BaseStitchIntTestHarness {
    ...
    public tearDown(): Promise<void> {
        return super.teardown().then(() =>
            Promise.all(this.clients.map((it) => { it.auth.logout() }))
        ).then(() => {});
    }
    
    get stitchBaseUrl(): string {
        const envVar = process.env[stitchBaseURLEnvVar];
        return envVar ? envVar : "http://localhost:9090";
    }
    ...
    public getAppClient(app: AppResponse): StitchAppClient {
        ...
        const client = Stitch.initializeAppClient(new StitchAppClientConfiguration.Builder()
    ...
    public registerAndLoginWithUserPass(
        app: App,
        ...
        return emailPassClient.registerWithEmail(email, pass)
            .then(() => app.userRegistrations.sendConfirmation(email))
            .then(conf => emailPassClient.confirmUser(conf.token, conf.tokenId))
            .then(() => client.auth.loginWithCredential(new UserPasswordCredential(email, pass)))
            .then(user => user.id);
    }
}