import { CoreStitchServiceClientImpl, Decoder } from "stitch-core";
import { anyOfClass, anything, capture, instance, mock, when } from "ts-mockito";
...
describe("CoreAwsSesServiceClient", () => {
    it("should send message", async () => {
        ...
        when(serviceMock.callFunctionInternal(anything(), anything(), anything()))
            .thenResolve(new AwsSesSendResult(expectedMessageId));
    
        const result = await client.sendEmail(toAddress, fromAddress, subject, body);
        expect(result.messageId).toEqual(expectedMessageId);

        const [funcNameArg, funcArgsArg, decoderArg]: any = 
            capture(serviceMock.callFunctionInternal).last();
        ...
        when(serviceMock.callFunctionInternal(anything(), anything(), anything())).thenThrow(
            new Error("whoops")
        );
