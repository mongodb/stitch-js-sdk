...
import { Anon, App, AppResponse, Custom, FunctionCreator, Userpass } from "stitch-admin";
import { AnonymousAuthProvider, AnonymousCredential, CustomAuthProvider, CustomCredential, UserPasswordAuthProvider, UserPasswordCredential, UserType } from "stitch-core";
import { UserPasswordAuthProviderClient } from "stitch-web";
...
describe("StitchAppClient", () => {
    it("should custom auth login", async () => {
        ...
        const jwt = sign({
            ...
            sub: "uniqueUserID",
        }, signingKey, { header: {"alg" : "HS256",
        "typ" : "JWT" }});

        const user = await client.auth.loginWithCredential(new CustomCredential(jwt));
    ...
    it("should follow multiple login semantics", async () => {
        ...
        await harness.addProvider(app as App, new Userpass(
        ...
        // check storage
        expect(client.auth.isLoggedIn).toBeTruthy();
        expect(client.auth.user!.loggedInProviderType).toEqual(AnonymousAuthProvider.TYPE)

        // login with email provider and make sure user ID is updated
        const emailUserId = await harness.registerAndLoginWithUserPass(app as App, client, "test@10gen.com", "hunter1");
        expect(emailUserId).not.toEqual(anonUser.id)

        // check storage
        expect(client.auth.isLoggedIn).toBeTruthy();
        expect(client.auth.user!.loggedInProviderType).toEqual(UserPasswordAuthProvider.TYPE)

        // login with email provider under different user and make sure user ID is updated
        const id2 = await harness.registerAndLoginWithUserPass(app as App, client, "test2@10gen.com", "hunter2");
    ...
    it("should link identity", async () => {
        ...
        const linkedUser = await anonUser.linkWithCredential(new UserPasswordCredential(email, password))

        expect(anonUser.id).toEqual(linkedUser.id)
        expect(linkedUser.loggedInProviderType).toEqual(UserPasswordAuthProvider.TYPE)
    ...
    it("should call function", async () => {
        ...
        const resultDoc = await (client.callFunction(
                "testFunction", [42, "hello"]
        ))
