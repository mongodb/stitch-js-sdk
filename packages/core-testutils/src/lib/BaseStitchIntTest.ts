...
export default abstract class BaseStitchIntTestHarness {
    ...
    public setup(): Promise<void> {
        // Verify stitch is up
        return fetch(this.stitchBaseUrl).then((result) => {
    ...
    public async createApp(appName: string = `test-${new ObjectID().toHexString()}`): Promise<Array<(App | AppResponse)>> {
    ...
    public async addProvider(app: App, config: ProviderConfig): Promise<AuthProviderResponse> {
        let authProviders;
    ...
    public async addService(app: App, type: string, config: ServiceConfig): Promise<Array<ServiceResponse | Service>> {
    ...
    public async addRule(svc: Service, config: RuleCreator): Promise<RuleResponse> {
