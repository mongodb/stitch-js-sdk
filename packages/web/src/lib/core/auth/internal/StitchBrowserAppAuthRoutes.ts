...
export default class StitchBrowserAppAuthRoutes extends StitchAppAuthRoutes {
    ...
    public getAuthProviderLinkRedirectRoute(
        ...
        return `${this.baseUrl}${this.getAuthProviderLoginRoute(credential.providerName)}?redirect=${encodeURI(redirectUrl)}&state=${state}&device=${this.uriEncodeObject(deviceInfo)}&link=true`
    }
